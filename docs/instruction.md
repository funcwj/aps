...
1. [kaldi feature](../aps/loader/am/kaldi.py) dataloader requires `feats.scp`, `utt2num_frames`, `text` (or other name based on the tokenization method).
...
2. [raw waveform](../aps/loader/am/raw.py) dataloader requires `wav.scp`, `utt2dur`, `text`.
    ...
    ```
    Note that aps also supports "pipe" or "archive" style input, so the `wav.scp` like:
    ```
    ...
    ```
    is also OK. The `text` is same as the one used in `am_kaldi` dataloader. `utt2dur` is also used to sort utterances to form the mini-batch that minimized the padding size.
