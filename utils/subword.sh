...
if [ $stage -eq 2 ]; then
  ...
  cat $text | awk '{$1=""; print $0;}' | spm_encode --model=$exp_dir/$mode.model --output_format=$encode > $exp_dir/enc.val
...
if [ $stage -eq 3 ]; then
  ...
  cat $text | awk '{$1=""; print $0;}' | spm_decode --model=$exp_dir/$mode.model --input_format=$decode > $exp_dir/dec.val
