...
if [ $end -ge 2 ] && [ $beg -le 2 ]; then
  ...
  local/prep_data.sh --mode $mode --spk $spk --sr $sr \
...
if [ $end -ge 4 ] && [ $beg -le 4 ]; then
  ...
  for name in dev test; do
    ./cmd/separate.py \
      ...
      --sr $sr_num \
      --device-id $gpu \
      $data_dir/$name/${eval_data}.scp \
      $cpt_dir/bss_${name}_${eval_data}
  done
fi
...
if [ $end -ge 5 ] && [ $beg -le 5 ]; then
  ...
  spk_list=$(seq $spk)
  for name in dev test; do
    eval_dir=bss_${name}_${eval_data}
    for index in $spk_list; do
      find $cpt_dir/$eval_dir -name "*.wav" | \
        awk -v ch=$index -F '/' '{printf("%s sox %s -t wav - remix %d |\n", $NF, $0, ch)}' | \
        sed "s:.wav::" > $cpt_dir/$eval_dir/s${index}.scp
    done
    ./cmd/compute_ss_metric.py \
      --sr $sr_num \
      --metric $metric \
      $data_dir/$name/s1.scp,$data_dir/$name/s2.scp \
      $cpt_dir/$eval_dir/s2.scp,$cpt_dir/$eval_dir/s2.scp
  done
fi
